...
\begin{abstract}This is a summary of the French monograph by \cite{CLD25}, which in particular develops a local theory of pluripotential functions on analytic spaces. All errors are mine and corrections are welcome. 
\end{abstract}
...
\subsection*{7.8: Harmonic charts. }Let $X$ be a \emph{boundaryless} tropical space of level $n$. We consider a special class of tropical charts. If $\omega$ is a form (as opposed to a $G$-form) on $X$, then Stokes' formula give \[d' [\omega] = [d'\omega].\] However, this is not the case for all $G$-forms. A particular class of $G$-forms where this equality of currents hold, is when it is tropicalized by a \emph{harmonic} tropical chart $u$ is \emph{harmonic}, which means the current $d' d'' [u]$ is zero. Such $G$-forms are called \emph{harmonically tropicalizable}, and forms a graded subalgebra of $\mathcal{A}_{X^G}(X)$. The forms that are locally harmonically tropicalizable form a sheaf $\mathcal{A}^{\bullet,\bullet}_{X, \mathrm{harm}}$. The inclusion $\mathcal{A}^{\bullet,\bullet}_{X, \mathrm{harm}} \hookrightarrow \mathcal{D}_X$ commutes with operators $d', d''$, thanks to the boundary current vanishing. 

\section*{Chapter 8: A Bedford--Taylor theory}

\subsection*{8.1: Plurisubharmonic functions. }

\subsection*{8.2: Tropical convexity. }

\subsection*{8.3: PSH functions and positive currents. }
